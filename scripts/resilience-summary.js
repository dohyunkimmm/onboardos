...
const reportPath = process.argv[2] || 'verification/resilience-playwright.json';
const release = JSON.parse(fs.readFileSync('release.json', 'utf8'));
const report = JSON.parse(fs.readFileSync(reportPath, 'utf8'));
const contract = release.resilienceContract;
if(!contract || contract.schemaVersion !== 1) throw new Error('resilienceContract schemaVersion 1 is required');

const scenarios = [];
function walkSuite(suite){
  for(const spec of suite.specs || []){
    const match = String(spec.title || '').match(/^\[(R\d+)\]\s*(.+)$/);
    if(!match) continue;
...
for(const id of contract.requiredScenarios){
  ...
  if(!row) throw new Error(`missing required resilience scenario ${id}`);
  if(row.result !== 'success') throw new Error(`resilience scenario ${id} failed`);
}
...
const summary = {
  ...
  sessionSchemaVersion:contract.sessionSchemaVersion,
...
if(!summary.allPassed) throw new Error('resilience suite did not fully pass');

const dir = 'verification';
fs.mkdirSync(dir,{recursive:true});
fs.writeFileSync(path.join(dir,'resilience-summary.json'), JSON.stringify(summary,null,2) + '\n');
const lines = [
  '# Resilience Evidence',
  '',
  `- Release: **ONBOARD·OS v${release.version} / ${release.freeze}**`,
  `- Session schema: \`${summary.sessionSchemaVersion}\``,
