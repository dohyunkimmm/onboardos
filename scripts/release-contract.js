...
function validateReleaseContract(){
  ...
  if(release.schemaVersion !== 4) fail(`schemaVersion must be 4, found ${release.schemaVersion}`);
  ...
  for(const critical of ['release.json', 'version.txt', 'integrity-assets.json', 'scripts/release-contract.js', 'scripts/release-evidence.js', 'scripts/public-assets.js']){
...
if(require.main === module){
  try {
    ...
    console.log(`Release contract PASS: ${release.product} v${release.version} · ${release.freeze} · canonical=${release.canonicalVersionSource} · integrity=${release.integrityManifest} · evidence=v${release.evidenceContract.schemaVersion}`);
  } catch (error) {
